...
const getSaldoNasabah = async () => {
    ...
    $('#formTarikSaldo #maximal-saldo').html(`_ _ _ _`);
    $('#saldo-uang').html('_ _');
...
const getDataProfileNasabah = async () => {
    ...
    if (httpResponse.status == 404) {
        ...
        // -- personal info --
        $('#personal-info #id_nas').html(`${dataNasabah.id}`);
...
const openModalTransaksi = (modalTitle) => {
    ...
    else if (modalTitle == 'tarik saldo') {
        $('#formTarikSaldo #maximal-saldo').html(`${modifUang(dataSaldo.uang)}`);
    }
...
const openModalEditSetor = async (id) => {
    ...
    if (httpResponse.status === 200) {
        ...
        let year  = date.toLocaleString("en-US",{year: "numeric"});
        let time  = date.toLocaleTimeString('en-IT', { hour: '2-digit', minute: '2-digit',hour12: false });
        let barang= httpResponse.data.data.barang;
