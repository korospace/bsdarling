 ...
 */
 $('#pemilik-saldo').on('click',function (e) {
    ...
    if ($(this).val() == "bst") {
        ...
        $(`#form-tarik-saldo`).attr('data-pemilik',"bst");
        $('#search-nasabah-wraper').addClass('d-none');
        ...
        $('#form-tarik-saldo #maximal-saldo-bst').removeClass('d-none');
        ...
        $('#barrier-search-nasabah').addClass('d-none');
        $(`#search-nasabah-wraper .input-group`).removeClass('opacity-6');
        $('#form-tarik-saldo #maximal-saldo-bst').addClass('d-none');
...
const tambahBaris = (event = false) => {
    ...
    <td class="py-2" style="border-right: 0.5px solid #E9ECEF;">
        <select id="kategori-berita-wraper" class="inputJenisSampah form-control form-control-sm py-1 pl-2 border-radius-sm" name="transaksi[slot${totalBaris+1}][id_sampah]" style="min-height: 38px" onchange="getHargaInOption(this,event,'${jenisForm}');" disabled>
    ...
    // <td class="py-2">
    //     <input type="text" class="inputHargaSampah form-control form-control-sm pl-2 border-radius-sm" name="transaksi[slot${totalBaris+1}][harga]" style="min-height: 38px" onkeyup="countHargaXjumlah(this,true);" data-harga="0" value="0">
    // </td>
...
const countHargaXjumlah = (el) =>{
    ...
    if (elInputJenis.value !== '') {
        if (jumlahKg !== '') {
            ...
            elInputHarga = el.parentElement.nextElementSibling.children[0];
            harga        = elInputHarga.getAttribute('data-harga');
...
// const countHargaXjumlah = (el,isHarga = false) =>{
//     var jumlahKg = (isHarga == false) ? el.value : el.parentElement.previousElementSibling.children[0].value;
//     let elInputJenis = el.parentElement.previousElementSibling.children[0];
...
//             } else {
//                 harga        = el.value;
//                 el.setAttribute('data-harga',harga)
//             }
...
const doTransaksi = async (el,event,method) => {
    ...
    if (validate(pemilikSaldo)) {
        ...
        if (httpResponse.status === 201) {
            ...
            if(method == 'tariksaldo' || method == 'tariksaldobst'){
    ...
    if (httpResponse.status === 404) {
        ...
        allTransaksi.forEach(t => {
            ...
            if (t.jenis_transaksi == 'penyetoran sampah') {
                ...
                color  = 'danger';
                jumlah = "- Rp "+modifUang(parseFloat(t.total_tarik).toFixed(0));
            } 
            else {
                color  = 'info';
...
const getDetailTransaksi = async (id) => {
    ...
    if (httpResponse.status === 200) {
        ...
        if (httpResponse.data.data.jenis_transaksi == 'penarikan saldo') {
            ...
            let jumlah     = 'Rp '+modifUang(parseFloat(httpResponse.data.data.jumlah_tarik).toFixed(0));
